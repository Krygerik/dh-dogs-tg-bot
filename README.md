...
FRIDA_INIT_SIGNATURE=Frida scripts have been injected.
```
...
| `INIT_SIGNATURE` | Строка лога завершения инициализации | `LogInit: Display: Engine is initialized...` |
| `INIT_TIMEOUT_MS` | Таймаут ожидания инициализации | `30000` |
| `FRIDA_PATH` | Путь к Frida инжектору (опц.) | `.\delete_me\python_loader.bat` |
| `FRIDA_INIT_SIGNATURE` | Сигнатура готовности Frida | `Frida scripts have been injected.` |
...
Сигнатура готовности задаётся через `FRIDA_INIT_SIGNATURE`.
...
├── scripts/                 # legacy-скрипты (не используются ботом)
