...
:: Start the Dread Hunger server (Departure map) and capture PID using PowerShell
for /f %%p in ('powershell -Command "& { $proc = Start-Process -FilePath 'DreadHungerServer-Win64-Shipping.exe' -ArgumentList 'Departure_Persistent?daysbeforeblizzard=3?maxplayers=8?thralls=2 -log' -PassThru; Write-Output $proc.Id }"') do (
    set PID=%%p
)
...
if exist "scripts_for_tg_bot/loader_for_departure_tg_bot.py" (
    echo [INFO] Running Python loader...
    python "scripts_for_tg_bot/loader_for_departure_tg_bot.py"
)
